...
class DailyMetrics(models.Model):
    ...
    delivered_orders = models.IntegerField("Pedidos Entregues", default=0)

    failed_orders = models.IntegerField("Pedidos Falhados", default=0)

    pending_orders = models.IntegerField("Pedidos Pendentes", default=0)

    # Métricas de Performance
    success_rate = models.DecimalField(
        "Taxa de Sucesso (%)", max_digits=5, decimal_places=2, default=0
    )
    ...
    total_revenue = models.DecimalField(
        "Receita Total", max_digits=10, decimal_places=2, default=0
    )

    total_bonuses = models.DecimalField(
        "Total de Bónus", max_digits=10, decimal_places=2, default=0
    )

    total_penalties = models.DecimalField(
        "Total de Penalidades", max_digits=10, decimal_places=2, default=0
    )

    # Métricas de Motoristas
    active_drivers_count = models.IntegerField("Motoristas Ativos", default=0)

    # Métricas de Veículos
    active_vehicles_count = models.IntegerField("Veículos Ativos", default=0)

    # Metadata
    calculated_at = models.DateTimeField("Calculado em", auto_now=True)
...
class VolumeForecast(models.Model):
    ...
    predicted_volume = models.IntegerField(
        "Volume Previsto", help_text="Número de pedidos previstos"
    )
    ...
    method = models.CharField(
        "Método", max_length=20, choices=FORECAST_METHODS, default="MA7"
    )
    ...
    def calculate_accuracy(self):
        ...
        if self.actual_volume is not None:
            ...
            if self.actual_volume > 0:
                ...
                self.accuracy = Decimal("0.00") if error > 0 else Decimal("100.00")
            self.save()
...
class PerformanceAlert(models.Model):
    ...
    alert_type = models.CharField(
        "Tipo de Alerta", max_length=30, choices=ALERT_TYPES, db_index=True
    )
    ...
    severity = models.CharField(
        "Severidade", max_length=10, choices=SEVERITY_LEVELS, default="WARNING"
    )
    ...
    description = models.TextField(
        "Descrição", help_text="Descrição detalhada do alerta"
    )
    ...
    acknowledged_by = models.CharField("Reconhecido por", max_length=100, blank=True)
...
class DriverPerformance(models.Model):
    ...
    month = models.DateField(
        "Mês", db_index=True, help_text="Primeiro dia do mês (YYYY-MM-01)"
    )

    # Métricas do Mês
    total_deliveries = models.IntegerField("Total de Entregas", default=0)

    successful_deliveries = models.IntegerField("Entregas Bem-Sucedidas", default=0)

    failed_deliveries = models.IntegerField("Entregas Falhadas", default=0)
    ...
    total_bonuses = models.DecimalField(
        "Bónus Totais", max_digits=10, decimal_places=2, default=0
    )

    total_penalties = models.DecimalField(
        "Penalidades Totais", max_digits=10, decimal_places=2, default=0
    )

    # Métricas Temporais
    days_worked = models.IntegerField("Dias Trabalhados", default=0)

    total_hours = models.DecimalField(
        "Horas Totais", max_digits=6, decimal_places=2, default=0
    )

    average_deliveries_per_day = models.DecimalField(
        "Média Entregas/Dia", max_digits=5, decimal_places=2, default=0
    )
    ...
    def __str__(self):
        return (
            f"{self.driver.user.get_full_name()} - " f"{self.month.strftime('%Y-%m')}"
        )

    @property
    def net_earnings(self):
        """Ganhos líquidos"""
        return self.total_earnings + self.total_bonuses - self.total_penalties
