...
class IntelligentDataDetector:
    ...
    def __init__(self):
        ...
        self.ranking_data_path = os.path.join(
            os.path.dirname(__file__), "ranking_data.json"
        )
    ...
    def _load_patterns(self) -> Dict[str, List[Dict]]:
        ...
        if (
            hasattr(self, "ranking_data")
            and self.ranking_data
            and "patterns" in self.ranking_data
        ):
                        ...
                        patterns[field].append(
                            {
                                ...
                                "confidence": pattern_info["score"]
                                / 10,  # Normaliza de 0-10 para 0-1
                                "usage_count": (
                                    pattern_info["usage_count"]
                                    if "usage_count" in pattern_info
                                    else 1
                                ),
                            }
                        )
        ...
        if not patterns["endereco"]:
            patterns["endereco"] = [
                {
                    "pattern": r"^(rua|r\.|av|av\.|avenida|travessa|largo|praça|estrada|via|quinta|beco)",
                    "confidence": 0.9,
                    "usage_count": 1,
                },
                {
                ...
                },  # código postal português
                {
                    ...
                    "confidence": 0.98,
                    "usage_count": 1,
                },  # formato completo com código postal
                {
                    "pattern": r"^.+\d+[a-zA-Z]?,.*\d{4}-\d{3}",
                    "confidence": 0.97,
                    "usage_count": 1,
                },  # rua + número + código postal
                {
                    "pattern": r".*ponte\s+de\s+lima.*",
                    "confidence": 0.8,
                    "usage_count": 1,
                },  # localidade comum
                {
                    "pattern": r".*riba\s+de\s+âncora.*",
                    "confidence": 0.8,
                    "usage_count": 1,
                },  # outra localidade
                {
                    ...
                    "confidence": 0.6,
                    "usage_count": 1,
                },  # qualquer coisa que não seja ID
            ]
        ...
        if not patterns["codigo_id"]:
            patterns["codigo_id"] = [
                ...
                {
                    "pattern": r"^#[EUJUW][a-zA-Z0-9]+_\d+",
                    "confidence": 0.98,
                    "usage_count": 1,
                },  # Formato dos IDs na captura
                {
                    "pattern": r"^#\d+",
                    "confidence": 0.97,
                    "usage_count": 1,
                },  # Formato #números como na captura
                {
                    "pattern": r"^[A-Z]\d+[A-Za-z]*",
                    "confidence": 0.7,
                    "usage_count": 1,
                },
                {
        ...
        if not patterns["data"]:
            patterns["data"] = [
                {
                    "pattern": r"^(today|hoje|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})$",
                    "confidence": 0.9,
                    "usage_count": 1,
                }
            ]
        ...
        for field_type, pattern_list in self.patterns.items():
            ...
            for pattern_info in pattern_list:
                ...
                if re.search(pattern, text, re.IGNORECASE):
                    # Usa usage_count como fator de boost para padrões frequentemente
                    # usados
                    boost = min(
                        1 + (pattern_info["usage_count"] * 0.1), 2.0
                    )  # Limita o boost a 2x
    ...
    def _process_structured_block(
        self, bloco: List[str], block_id: int
    ) -> Optional[Dict]:
        """
        ...
        """
        print(
            f"_process_structured_block: processando bloco com {len(bloco)} linhas: {bloco}"
        )

        # Precisa ter pelo menos 2 linhas para ser válido (endereço + ID mínimo)
        if len(bloco) < 2:
            print(
                f"_process_structured_block: bloco muito pequeno ({len(bloco)} linhas)"
            )
            return None
        ...
        for i, linha in enumerate(bloco):
            ...
            if (
                not flags_encontrados["endereco"]
                and not linha_limpa.startswith("#")
                and not linha_limpa.isdigit()
            ):
            ...
            if not flags_encontrados["hora"] and re.match(
                r"^\d{1,2}:\d{2}$", linha_limpa
            ):
            ...
            if not flags_encontrados["horario"] and re.match(
                r"^\d{1,2}:\d{2}\s*[-–]\s*\d{1,2}:\d{2}", linha_limpa
            ):
            ...
            if not flags_encontrados["data"]:
                if linha_limpa.lower() == "today" or re.match(
                    r"^\d{1,2}[/-]\d{1,2}", linha_limpa
                ):
            ...
            if not flags_encontrados["quantidade"] and re.match(
                r"^\d{1,2}$", linha_limpa
            ):
        ...
        if re.match(r"^\d{4}-\d{3}", endereco):
            ...
            for parte in partes:
                if re.match(r"^\d{4}-\d{3}$", parte):
                    ...
                    codigo_postal and not localidade
                ):  # A parte após o código postal é geralmente a localidade
                    localidade = parte
                elif not rua_numero:
                    rua_numero = parte
    ...
    def _process_block_intelligently(
        self, bloco: List[str], block_id: int
    ) -> Optional[Dict]:
        ...
        if (
            id_index == -1
        ):  # Se não encontrar ID, tenta processar usando detecção de campo
            ...
            if id_index > 0:
                ...
                # Isso ajuda a IA a aprender que IDs também podem conter informações
                # relacionadas ao endereço
            ...
            if id_index + 1 < len(bloco) and bloco[id_index + 1].strip():
                ...
                if (
                    bloco[id_index + 1]
                    .lower()
                    .startswith(("rua", "av", "avenida", "travessa", "r.", "via"))
                ):
                ...
                for offset, field in enumerate(bloco[start_idx:]):
                    ...
                    elif offset == 2 and field.lower() in [
                        "today",
                        "hoje",
                    ]:  # Data
                        resultado["data"] = field
                        resultado["confidence_scores"]["data"] = 0.9
                    elif offset == 3 and re.match(
                        r"^\d{1,2}:\d{2}\s*-\s*\d{1,2}:\d{2}$", field
                    ):  # Horário
                        resultado["horario"] = field
                        resultado["confidence_scores"]["horario"] = 0.9
                    elif offset == 4 and (
                        field.endswith("L") or re.match(r"^\d+(\.\d+)?", field)
                    ):  # Litros
                        ...
                        if pattern_tuple:
                            ...
                            for p in self.ranking_data["patterns"][field]:
                                if p["pattern"] == pattern:
                                    p["score"] = min(
                                        10, p["score"] + 0.5
                                    )  # Aumenta score (max 10)
                                    p["usage_count"] = p.get("usage_count", 0) + 1
                                    p["last_used"] = datetime.now().strftime(
                                        "%Y-%m-%d %H:%M:%S"
                                    )
                                    found = True
                                    break
        ...
        if field_type == "endereco":
            ...
            if re.search(r"\d{4}-\d{3}", value):
                return (
                    r".*\d{4}-\d{3}.*",
                    9.5,
                )  # Padrão de código postal português
            ...
            if words[0] in [
                ...
                return (
                    f"^{re.escape(words[0])}\\s",
                    9,
                )  # Padrão para início do endereço
        ...
        try:
            ...
            try:
                learning_pattern = LearningPattern.objects.get(
                    pattern_type=field_type, pattern_value=pattern
                )
                learning_pattern.usage_count += 1
                learning_pattern.confidence = min(
                    0.95, learning_pattern.confidence + 0.05
                )
    ...
    def calculate_accuracy(self):
        ...
        for field, patterns in self.ranking_data.get("patterns", {}).items():
            for pattern in patterns:
                patterns_count += 1
                weighted_score += pattern["score"] * min(
                    2, pattern.get("usage_count", 1) / 5 + 1
                )
        ...
        report = {
            ...
            "ranking_stats": {
                ...
                "patterns_learned": sum(
                    len(patterns)
                    for patterns in self.ranking_data.get("patterns", {}).values()
                ),
                "corrections_made": len(self.ranking_data.get("corrections", [])),
                "examples_collected": sum(
                    len(examples)
                    for examples in self.ranking_data.get("examples", {}).values()
                ),
            },
        }
