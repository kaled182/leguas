...
class GeoAPIValidator:
    ...
    def _extract_street_name(self, address: str) -> Optional[str]:
        ...
        patterns = [
            (
                r"^(rua|r\.|avenida|av\.|travessa|tv\.|largo|lg\.|praça|pc\.|pç\.|alameda)\s+(.+?)(?:\s+\d+)?$",
                2,
            ),
            (
                r"^(.+?)(?:\s+\d+)?$",
                1,
            ),  # Fallback: pega todo o texto até um número
        ]
    ...
    async def validate_address(self, address: str) -> GeoValidationResult:
        ...
        try:
            ...
            result = GeoValidationResult(
                ...
                validated_address=(
                    geo_info.get("formatted_address", address) if geo_info else address
                ),
                coordinates=(
                    tuple(geo_info.get("coordinates", data.get("centro", [0, 0])))
                    if geo_info
                    else tuple(data.get("centro", [0, 0]))
                ),
    ...
    async def bulk_validate_addresses(
        self, addresses: List[str]
    ) -> Dict[str, GeoValidationResult]:
        """
                    ...
                    if response.status == 200:
                        ...
                        if data and "features" in data and len(data["features"]) > 0:
                            ...
                            return {
                                ...
                                "formatted_address": feature.get("properties", {}).get(
                                    "formatted", address
                                ),
