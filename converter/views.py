...
def converter_view(request):
    ...
    if request.method != "POST":
        ...
        for field_type in [
            ...
            ai_stats["patterns"][field_type] = LearningPattern.objects.filter(
    ...
    if action == "parse":
        ...
        try:
            ...
            for field_type in [
                ...
                stats["patterns"][field_type] = LearningPattern.objects.filter(
                    pattern_type=field_type
                ).count()

            # Padrões aprendidos com ranking
            stats["ranking_patterns"] = {}
            for field_type, patterns in detector.ranking_data.get(
                "patterns", {}
            ).items():
        ...
        for column in ws.columns:
            ...
            adjusted_width = min(
                max_length + 3, 50
            )  # +3 para dar um pouco mais de espaço
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="lista_convertida_{timezone.now().strftime("%Y%m%d_%H%M")}.xlsx"'
        )
