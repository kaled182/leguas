...
async def validate_addresses(request):
    ...
    try:
        ...
        if not request.content_type.startswith("application/json"):
            ...
            return JsonResponse(
                {"error": "Content-Type deve ser application/json"}, status=400
            )
        ...
        try:
            ...
            logger.info(
                f"Validação concluída com sucesso para {len(results)} endereços"
            )
        except Exception as e:
        ...
        return JsonResponse(
            {"error": f"Erro ao processar validação: {str(e)}"}, status=500
        )
