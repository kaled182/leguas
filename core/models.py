...
class Partner(models.Model):
    ...
    pudo_enabled = models.BooleanField(
        "PUDO activo neste parceiro",
        default=False,
        help_text=(
            "Se ativo, entregas com Delivery Type=PUDO no EPOD são "
            "remuneradas pelas regras PUDO em vez do preço padrão."
        ),
    )
    pudo_first_delivery_price = models.DecimalField(
        "PUDO — 1ª entrega no mesmo PUDO (€)",
    ...
    pudo_additional_delivery_price = models.DecimalField(
        "PUDO — entregas adicionais no mesmo PUDO (€)",
        max_digits=6, decimal_places=4,
        default=Decimal("0.2000"),
        help_text=(
            "Pago por cada encomenda extra no MESMO PUDO além da "
            "primeira. Fórmula total: 1ª + (N-1) × adicional."
        ),
    )
    pudo_fake_delivery_penalty = models.DecimalField(
        "PUDO — penalização Fake Delivery (€)",
        max_digits=6, decimal_places=4,
        default=Decimal("1.3000"),
        help_text=(
            "Descontado ao motorista quando uma entrega marcada como "
    ...
    pudo_geo_tolerance_meters = models.PositiveIntegerField(
        ...
        default=200,
        help_text=(
            "Distância máxima em metros entre a localização do PUDO "
            "(receiver_lat/lng) e o local onde o motorista marcou "
            "Delivered (actual_lat/lng). Acima desta tolerância a "
            "entrega é sinalizada como SUSPEITA de Fake Delivery — "
            "operador confirma manualmente antes de gerar penalização."
        ),
    )
