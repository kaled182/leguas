...
def driversmanager_view(request):
    ...
    welcome_name = (
        f"{driver_access.first_name} {driver_access.last_name}".strip() or "Motorista"
    )

    to_attempt_count = sum(
        route.get_orders(date=date).filter(simplified_order_status="to_attempt").count()
        for route in [DriverRoute(d) for d in [driver]]
    )
    picked_up_count = sum(
        route.get_orders(date=date).filter(simplified_order_status="picked_up").count()
        for route in [DriverRoute(d) for d in [driver]]
    )
    delivered_count = sum(
        route.get_orders(date=date).filter(simplified_order_status="delivered").count()
        for route in [DriverRoute(d) for d in [driver]]
    )
    failed_count = sum(
        route.get_orders(date=date).filter(simplified_order_status="failed").count()
        for route in [DriverRoute(d) for d in [driver]]
    )
    ...
    for idx, order in enumerate(orders_qs, start=1):
        ws.append(
            [
                ...
                (
                    order.intended_delivery_date.strftime("%d/%m/%Y")
                    if order.intended_delivery_date
                    else ""
                ),
