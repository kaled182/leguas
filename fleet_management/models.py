...
class Vehicle(models.Model):
    ...
    year = models.IntegerField("Ano", help_text="Ano de fabrico")

    vehicle_type = models.CharField(
        "Tipo de Veículo", max_length=20, choices=VEHICLE_TYPES, default="VAN"
    )
    ...
    insurance_policy_number = models.CharField(
        "Número da Apólice", max_length=100, blank=True
    )
    ...
    def is_available(self):
        ...
        return (
            self.status == "ACTIVE"
            and self.is_inspection_valid
            and self.is_insurance_valid
        )
...
class VehicleAssignment(models.Model):
    ...
    end_time = models.TimeField("Hora de Fim", null=True, blank=True)
    ...
    def __str__(self):
        return (
            f"{self.vehicle.license_plate} → {self.driver.nome_completo} ({self.date})"
        )
...
class VehicleMaintenance(models.Model):
    ...
    maintenance_type = models.CharField(
        "Tipo de Manutenção", max_length=20, choices=MAINTENANCE_TYPES
    )

    description = models.TextField(
        "Descrição", help_text="Descrição detalhada do serviço realizado"
    )
    ...
    invoice_number = models.CharField("Número da Fatura", max_length=100, blank=True)

    # Odómetro quando feito
    odometer_at_service = models.IntegerField(
        "Quilómetros no Serviço", null=True, blank=True
    )
...
class VehicleIncident(models.Model):
    ...
    incident_date = models.DateField("Data do Incidente", db_index=True)
