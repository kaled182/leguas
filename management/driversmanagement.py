...
def drivers_management_view(request):
    ...
    if request.GET.get("export") == "xlsx":
        ...
        for row_num, access in enumerate(
            DriverAccess.objects.select_related("driver").all(), start=1
        ):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="acessos_motoristas.xlsx"'
        )
        return response
    ...
    if request.GET.get("export") == "pdf":
        ...
        response["Content-Disposition"] = (
            'attachment; filename="acessos_motoristas.pdf"'
        )
        buffer = io.BytesIO()
    ...
    # Listagem
    driver_accesses = (
        DriverAccess.objects.select_related("driver")
        .all()
...
def edit_driver_access(request, access_id):
    ...
    return render(
        request,
        "edit_driver_access.html",
        {"access": access, "drivers": drivers},
    )
