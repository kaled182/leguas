...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Início da semana: {week_start} ({week_start.strftime('%A')})"
        )
        ...
        if week_efficiency_rates:
            ...
            self.stdout.write("-" * 50)
            self.stdout.write(
                f"Taxas diárias: {[f'{rate:.1f}%' for rate in week_efficiency_rates]}"
            )
            self.stdout.write(f"Soma: {sum(week_efficiency_rates):.1f}")
            self.stdout.write(f"Quantidade de dias: {len(week_efficiency_rates)}")
            self.stdout.write(
                f"Média: {sum(week_efficiency_rates):.1f} / {len(week_efficiency_rates)} = {week_avg:.1f}%"
            )
        else:
            week_efficiency = "0.0%"
