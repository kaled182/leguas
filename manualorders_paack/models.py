...
class ManualCorrection(models.Model):
    ...
    correction_type = models.CharField(
        "Tipo de Correção", max_length=4, choices=CORRECTION_TYPES
    )
    status = models.CharField(
        "Status", max_length=20, choices=STATUS_CHOICES, default="delivered"
    )
    reason = models.TextField("Motivo")
    driver = models.ForeignKey(
        Driver, on_delete=models.PROTECT, related_name="manual_corrections"
    )
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="manual_corrections"
    )
    dispatch = models.ForeignKey(
        Dispatch, on_delete=models.CASCADE, related_name="manual_corrections"
    )

    # Campos para rastreabilidade e controle
    original_status = models.CharField(
        "Status Original", max_length=20, blank=True, null=True
    )
    original_driver = models.ForeignKey(
        Driver,
        on_delete=models.PROTECT,
        blank=True,
        null=True,
        related_name="original_manual_corrections",
    )
    correction_date = models.DateField(
        "Data da Correção", help_text="Data para qual a correção se aplica"
    )
    quantity = models.PositiveIntegerField(
        "Quantidade",
        default=1,
        help_text="Quantidade de entregas adicionadas/removidas",
    )
    ...
    is_active = models.BooleanField(
        "Ativo", default=True, help_text="Se False, a correção foi desfeita"
    )
