...
class OrderQuerySet(models.QuerySet):
    ...
    def with_multiple_attempts(self):
        return self.annotate(attempt_count=Count("deliveryattempt")).filter(
            attempt_count__gt=1
        )
    ...
    def with_delivery_stats(self, start_date=None, end_date=None):
        ...
        if start_date and end_date:
            queryset = queryset.filter(
                dispatch__order__intended_delivery_date__range=[
                    start_date,
                    end_date,
                ]
            )
...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.is_failed = self.simplified_order_status in [
            "failed",
            "undelivered",
        ]
...
class Driver(models.Model):
    ...
    def get_deliveries_by_date(self, date):
        ...
        return Order.objects.filter(
            dispatch__driver=self, actual_delivery_date=date, is_delivered=True
        )

    def get_orders_by_date(self, date):
        """Retorna todas as ordens designadas ao motorista em uma data específica"""
        return Order.objects.filter(dispatch__driver=self, intended_delivery_date=date)
    ...
    def get_daily_stats(self, date):
        ...
        return {
            ...
            "success_rate": (
                (deliveries.count() / orders.count() * 100) if orders.count() > 0 else 0
            ),
        }
...
class Dispatch(models.Model):
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="dispatch"
    )
