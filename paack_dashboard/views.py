...
class DashboardDataService:
    ...
    def _get_date_filter_for_queries(self):
        ...
        if self.date_range_mode:
            ...
            start_datetime = timezone.datetime.combine(
                self.start_date_obj, timezone.datetime.min.time()
            )
            end_datetime = timezone.datetime.combine(
                self.end_date_obj, timezone.datetime.max.time()
            )
            start_datetime = timezone.make_aware(start_datetime)
            end_datetime = timezone.make_aware(end_datetime)
            return {"dispatch_time__range": [start_datetime, end_datetime]}
        else:
            # Para data única, usar range do dia completo
            from django.utils import timezone

            start_datetime = timezone.datetime.combine(
                self.filter_date_obj, timezone.datetime.min.time()
            )
            end_datetime = timezone.datetime.combine(
                self.filter_date_obj, timezone.datetime.max.time()
            )
            start_datetime = timezone.make_aware(start_datetime)
            end_datetime = timezone.make_aware(end_datetime)
            return {"dispatch_time__range": [start_datetime, end_datetime]}
    ...
    def _get_orders_by_dispatch_date(self):
        ...
        if self.date_range_mode:
            ...
            dispatch_counts = (
                apiDispatch.objects.filter(**date_filter)
                ...
                .annotate(
                    ...
                    failed=Count(
                        Case(
                            When(
                                order__simplified_order_status__in=[
                                    "failed",
                                    "undelivered",
                                ],
                                then=1,
                ...
                .annotate(
                    success_rate=Case(
                        When(
                            total__gt=0,
                            then=(100.0 * F("delivered") / F("total")),
                        ),
                        default=0,
    ...
    def get_dispatches_by_date(self):
        ...
        date_filter = self._get_date_filter_for_queries()
        return apiDispatch.objects.filter(**date_filter).select_related(
            "driver", "order"
        )
    ...
    def get_dispatch_metrics(self):
        ...
        metrics = apiDispatch.objects.filter(**date_filter).aggregate(
            total=Count("id"),
            delivered=Count(
                Case(
                    When(order__status="delivered", then=1),
            ...
            recovered=Count(
                Case(When(recovered=True, then=1), output_field=IntegerField())
            ),
        )
        ...
        if metrics["total"] > 0:
            metrics["success_rate"] = round(
                (metrics["delivered"] / metrics["total"]) * 100, 2
            )
        else:
    ...
    def get_best_driver(self):
        ...
        best_driver_data = (
            ...
            .annotate(
                driver_success_rate=Case(
                    When(
                        total_dispatches__gt=0,
                        then=(100.0 * F("total_delivered") / F("total_dispatches")),
                    ),
                    default=0,
                    output_field=FloatField(),
                )
            ...
            .filter(
                total_dispatches__gte=5
            )  # Mínimo de 5 entregas para ser considerado
        ...
        best_driver = (
            apiDispatch.objects.filter(driver=best_driver_data["driver"], **date_filter)
            .select_related("driver")
            .first()
        )
            ...
            formatted_distribution.append(
                {
                    ...
                    "success_rate": (
                        round(
                            (hour_data["delivered"] / hour_data["total"] * 100),
                            2,
                        )
    ...
    def _extract_driver_name(self, full_name):
        ...
        for i, part in enumerate(parts):
            ...
            # Verifica se parece um nome próprio (primeira letra maiúscula, resto
            # minúsculo)
        ...
        if nome_candidato:
            # Remover possíveis sufixos (códigos no final que são todas letras
            # maiúsculas)
            if (
                nome_candidato
                and nome_candidato[-1].isupper()
                and len(nome_candidato[-1]) <= 3
            ):
                nome_candidato.pop()
            ...
            .annotate(
                success_rate=Case(
                    When(
                        ...
                        then=(100.0 * F("deliveries") / F("total_attempts")),
                    ),
                    default=0,
                    output_field=FloatField(),
                )
    ...
    def get_driver_profile_image(self, driver_id):
        ...
        try:
            ...
            print(
                f"Erro ao buscar imagem de perfil para motorista {driver_id}: {str(e)}"
            )
            return None
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get("sync") == "true":
            ...
            if sync_result.get("success", False):
                messages.success(
                    self.request,
                    f"Dados atualizados com sucesso! Última atualização: {timezone.now().strftime('%H:%M:%S')}",
                )
                ...
                messages.warning(
                    self.request,
                    f"Você pode estar vendo dados desatualizados. {error_msg}",
                )
        ...
        if data_service.date_range_mode:
            context["start_date_formatted"] = data_service.start_date_obj.strftime(
                "%d/%m/%Y"
            )
            context["end_date_formatted"] = data_service.end_date_obj.strftime(
                "%d/%m/%Y"
            )
            if data_service.start_date_obj == data_service.end_date_obj:
                context["current_date"] = data_service.start_date_obj.strftime(
                    "%d/%m/%Y"
                )
            else:
                context["current_date"] = (
                    f"{data_service.start_date_obj.strftime('%d/%m')} a {data_service.end_date_obj.strftime('%d/%m/%Y')}"
                )
        else:
        ...
        context["total_orders_description"] = (
            data_service.get_total_orders_description()
        )
        ...
        context["top_drivers_today"] = (
            context["driver_success_chart"][:10]
        ...
        # Limpamos os dados para serialização - usando dict mais simples para
        ...
        context["driver_success_chart_json"] = json.dumps(
            ...
            separators=(
                ",",
                ":",
            ),  # Remover espaços extras para compactar o JSON
        )
...
class DebugWeeklyEfficiencyView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        while current_day <= week_end:
            # Cálculos do dia com apiDispatch para manter consistência com o resto do
            # dashboard
            day_deliveries = apiDispatch.objects.filter(
                dispatch_time__date=current_day, order__status="delivered"
            ).count()
            ...
            if day_total > 0:
                ...
                if (
                    day_deliveries + day_fails > 0
                ):  # Somente se houver entregas concluídas
...
def driver_profile_image(request, driver_id):
    ...
    try:
        ...
        if driver_access and driver_access.profile_picture:
            return FileResponse(
                driver_access.profile_picture.open(), content_type="image/jpeg"
            )
