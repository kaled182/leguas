...
class PostalZone(models.Model):
    ...
    is_urban = models.BooleanField(
        "Zona Urbana", default=True, help_text="Se zona é urbana (vs. rural)"
    )
...
class PartnerTariff(models.Model):
    ...
    def __str__(self):
        return f"{self.partner.name} - {self.postal_zone.name} " f"(€{self.base_price})"
        ...
        if self.valid_until and self.valid_from:
            if self.valid_until < self.valid_from:
                raise ValidationError(
                    {"valid_until": ("Data final deve ser posterior à data inicial")}
                )
    ...
    def get_tariff_for_order(cls, partner, postal_code, delivery_date=None):
        ...
        tariffs = (
            ...
            .filter(
                ...
                | models.Q(valid_until__gte=delivery_date)
            )
            .order_by("-valid_from")
        )
