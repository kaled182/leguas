...
class DriverShiftAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Motorista e Data", {"fields": ("driver", "date", "created_by")}),
        ("Zonas Atribuídas", {"fields": ("assigned_postal_zones",)}),
        ("Horário Previsto", {"fields": ("start_time", "end_time")}),
        (
            "Horário Real",
            {
                "fields": (
    ...
    def formatted_stats(self, obj):
        ...
        color = (
            ...
            else "orange" if obj.success_rate >= 70 else "red"
        )
    ...
    def whatsapp_sent(self, obj):
        ...
        return format_html('<span style="color: orange;">⚠ Não enviada</span>')
    ...
    def send_whatsapp_notifications(self, request, queryset):
        ...
        self.message_user(
            request, f"{sent_count} notificação(ões) WhatsApp enviada(s)."
        )
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(request, f"{updated} turno(s) marcado(s) como concluído.")
