...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if self.test_mode:
            self.stdout.write(self.style.WARNING("🧪 MODO DE TESTE - Nenhum relatório será enviado"))
        
        if not options['force']:
            if not is_working_hours:
                self.stdout.write(f"⏰ Fora do horário de trabalho ({self.start_hour}h-{self.end_hour}h)")
                return
            
            if not is_correct_minute:
                self.stdout.write(f"⏱️ Aguardando horário correto (xx:00 ou xx:30). Atual: {current_minute} min")
                return
