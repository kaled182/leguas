...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--year',
            type=int,
            help='Ano. Se não informado, usa ano atual'
        )
        parser.add_argument(
            '--partner-id',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if partner_id:
            ...
            if not dry_run:
                ...
                self.stdout.write('\n' + '='*60)
                self.stdout.write(self.style.SUCCESS(f'✅ {len(invoices)} invoices criados'))
        ...
        if overdue:
            self.stdout.write(self.style.WARNING(f'⚠️ {len(overdue)} invoices atrasados:'))
            for inv in overdue:
