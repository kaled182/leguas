...
def plans_list(request):
    ...
    if request.GET.get("driver_id"):
        qs = qs.filter(driver_id=request.GET["driver_id"])
    if request.GET.get("client"):
        qs = qs.filter(client=request.GET["client"])
    if request.GET.get("area_code"):
        qs = qs.filter(area_code=request.GET["area_code"])
...
def plan_update(request, plan_id: int):
    ...
    for field in ["client", "area_code", "is_active"]:
        if field in body:
            setattr(plan, field, body[field] or None)
    if "starts_on" in body:
        plan.starts_on = _parse_date(body["starts_on"])
    if "ends_on" in body:
        plan.ends_on = _parse_date(body["ends_on"])
    if "base_fixed" in body:
        plan.base_fixed = Decimal(str(body["base_fixed"] or 0))
...
def plan_preview(request):
    ...
    if driver_id:
        ...
        if not plan_id:
            return HttpResponseBadRequest("plan_id ou driver_id é obrigatório")
        plan = CompensationPlan.objects.get(id=plan_id)
