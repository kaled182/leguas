...
from .models import CompensationPlan, SettlementRun, ThresholdBonus


def _pick_plan(
    driver, client: Optional[str], area: Optional[str], day: date
) -> Optional[CompensationPlan]:
    q = CompensationPlan.objects.filter(
        driver=driver, is_active=True, starts_on__lte=day
    ).filter(models.Q(ends_on__isnull=True) | models.Q(ends_on__gte=day))
    return (
        q.filter(client=client, area_code=area).first()
...
def _apply_pkg_rates(plan: Optional[CompensationPlan], delivered: int) -> Decimal:
    ...
    for r in rates:
        if (delivered >= r.min_delivered) and (
            r.max_delivered is None or delivered <= r.max_delivered
        ):
...
def compute_payouts(
    ...
    qs = SettlementRun.objects.filter(
        run_date__gte=period_from, run_date__lte=period_to
    )
    if client:
        qs = qs.filter(client=client)
    if area:
        qs = qs.filter(area_code=area)

    agg = qs.values("driver_id", "driver__name").annotate(
    ...
    for row in agg:
        ...
        sample_run = (
            SettlementRun.objects.filter(driver_id=row["driver_id"])
            .order_by("-run_date")
            .first()
        )
        ...
        results.append(
            {
                ...
                "media_liq_por_pacote": (
                    float(liquido / delivered) if delivered else 0.0
                ),
            }
        )
