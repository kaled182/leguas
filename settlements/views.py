...
def financial_dashboard(request):
    ...
    settlements_stats = {
        ...
        "pending": DriverSettlement.objects.filter(
...
def settlement_detail(request, settlement_id):
    ...
    settlement = get_object_or_404(
        DriverSettlement.objects.select_related("driver", "partner").prefetch_related(
            "claims"
        ),
        id=settlement_id,
    )
...
def settlement_download_pdf(request, settlement_id):
    ...
    period = (
        f"S{settlement.week_number}"
        if settlement.period_type == "WEEKLY"
        else f"M{settlement.month_number}"
    )
