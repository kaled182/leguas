...
from .models import ConfigurationAudit, CronJobExecution, SystemConfiguration
...
class SystemConfigurationAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
        "configured_at",
        "updated_at",
        "cron_metrics_last_run",
        "cron_metrics_last_status",
        "cron_forecasts_last_run",
        "cron_forecasts_last_status",
        "cron_alerts_last_run",
        "cron_alerts_last_status",
        "cron_jobs_summary",
    )

    fieldsets = (
        ("Informações da Empresa", {"fields": ("company_name", "logo")}),
        (
        ...
        (
            "⏰ Cron Jobs - Analytics",
            {
                "fields": (
                    "cron_jobs_summary",
                    (
                        "cron_metrics_enabled",
                        "cron_metrics_schedule",
                        "cron_metrics_backfill_days",
                    ),
                    ("cron_metrics_last_run", "cron_metrics_last_status"),
                    ("cron_forecasts_enabled", "cron_forecasts_schedule"),
                    (
                        "cron_forecasts_days_ahead",
                        "cron_forecasts_method",
                        "cron_forecasts_best_only",
                    ),
                    ("cron_forecasts_last_run", "cron_forecasts_last_status"),
                    ("cron_alerts_enabled", "cron_alerts_schedule"),
                    (
                        "cron_alerts_check_days",
                        "cron_alerts_send_notifications",
                    ),
        ...
        ("Sistema", {"fields": ("configured", "configured_at", "updated_at")}),
    )
...
class CronJobExecutionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Parâmetros", {"fields": ("parameters",), "classes": ("collapse",)}),
        (
            "Logs",
            {"fields": ("output_log", "error_log"), "classes": ("collapse",)},
        ),
    )

    def job_type_display(self, obj):
        """Exibe tipo do job com emoji"""
        icons = {"metrics": "📊", "forecasts": "📈", "alerts": "🔔"}
    ...
    def results_summary(self, obj):
        ...
        if obj.records_created > 0:
            parts.append(
                f'<span style="color: #28a745;">✨ {obj.records_created}</span>'
            )
        if obj.records_updated > 0:
            parts.append(
                f'<span style="color: #17a2b8;">🔄 {obj.records_updated}</span>'
            )
        if obj.records_skipped > 0:
            parts.append(
                f'<span style="color: #6c757d;">⏭️ {obj.records_skipped}</span>'
            )
