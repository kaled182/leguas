...
def _fallback_config() -> RuntimeConfig:
    ...
    return RuntimeConfig(
        ...
        optical_rx_warning_threshold=_float_setting(
            "OPTICAL_RX_WARNING_THRESHOLD", -24.0
        ),
        optical_rx_critical_threshold=_float_setting(
...
def get_runtime_config() -> RuntimeConfig:
    ...
    return RuntimeConfig(
        ...
        zabbix_api_password=record.zabbix_password
        or getattr(settings, "ZABBIX_API_PASSWORD", ""),
        zabbix_api_key=record.zabbix_api_key or getattr(settings, "ZABBIX_API_KEY", ""),
        google_maps_api_key=record.maps_api_key
        ...
        or getattr(settings, "GDRIVE_FOLDER_ID", ""),
        ...
        smtp_auth_mode=record.smtp_auth_mode
        or getattr(settings, "SMTP_AUTH_MODE", "password"),
        ...
        sms_test_recipient=record.sms_test_recipient
        or getattr(settings, "SMS_TEST_RECIPIENT", ""),
        sms_test_message=record.sms_test_message
        or getattr(settings, "SMS_TEST_MESSAGE", ""),
        ...
        mapbox_style=record.mapbox_style
        or getattr(settings, "MAPBOX_STYLE", "streets-v12"),
        ...
        esri_basemap=record.esri_basemap
        or getattr(settings, "ESRI_BASEMAP", "topo-vector"),
...
class RuntimeSettings:
    """Wrapper class for runtime settings"""

    def get_map_settings(self):
        """Get map-related settings"""
        ...
        return {
            ...
            "default_lat": config.map_default_lat,
            "default_lng": config.map_default_lng,
        }
