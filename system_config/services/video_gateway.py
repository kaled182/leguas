...
from system_config.models import MessagingGateway
...
def _get_webrtc_manifest_timeout(config: Optional[Dict[str, object]] = None) -> float:
    ...
    if candidate is None:
        fallback = getattr(
            settings, "VIDEO_WEBRTC_HLS_GRACE_SECONDS", None
        ) or os.environ.get("VIDEO_WEBRTC_HLS_GRACE_SECONDS")
...
def build_playback_url(gateway: MessagingGateway) -> str:
    ...
    Se o preview atual j� for HLS, retorna-o. Caso contr�rio, tenta construir
    um manifesto HLS p�blico usando a base configurada e a stream key.
    """
    ...
    if stream_url.lower().startswith("http"):
        # Streams HTTP diretos n�o passam pelo transmuxer; usar URL original.
...
def _get_webrtc_public_base_url(
...
def stop_stream_for_gateway(
    ...
    try:
        ...
        if response.status_code not in {200, 404}:
            logger.warning("Falha ao encerrar stream %s: %s", stream_key, response.text)
...
def _wait_for_manifest(
    stream_key: str, *, timeout: float = 30.0, interval: float = 0.6
) -> None:
...
def ensure_stream_for_gateway(
    ...
    *,
    ...
    if use_webrtc:
        # MediaMTX WebRTC player page (barra final obrigat�ria)
    ...
    # Sempre preferir HLS para playback quando dispon�vel; preview_url fica
    # para player WebRTC/iframe
    playback_url = hls_candidate or preview_url

    # Verificar se stream j� est� ativo
    stream_active = False
    try:
    ...
    if stream_active:
        logger.debug("Stream %s j� est� ativo, reutilizando.", stream_key)
        # Garantir que preview_url e restream_key est�o salvos
        if (
        ...
        hls_ready = True
        # Validar manifesto mesmo se stream j� estiver ativo
        if wait_ready and should_wait_hls:
            try:
                ...
                if use_webrtc:
                    ...
                    stream_active = False
        if not hls_ready:
            # Manter playback apontando para HLS mesmo que n�o esteja pronto;
            # o frontend far� retries at� o manifesto existir.
            playback_url = hls_candidate or preview_url
    ...
    if not stream_active:
        ...
        try:
            ...
            raise VideoGatewayError(
                "Nao foi possivel iniciar o processo de restream."
            ) from exc
