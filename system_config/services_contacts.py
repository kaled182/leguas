...
class ContactImportService:
    ...
    def import_from_file(self, file) -> ImportHistory:
        ...
        try:
            ...
            logger.info(f"Importação concluída: {successful} sucesso, {failed} falhas")
    ...
    def _read_csv(self, file) -> List[Dict]:
        ...
        for row in reader:
            ...
            normalized_row = {
                self._normalize_key(k): v.strip() if v else "" for k, v in row.items()
            }
    ...
    def _process_rows(
        self, rows: List[Dict], import_history: ImportHistory
    ) -> Tuple[int, int]:
        """
        ...
        for idx, row in enumerate(rows, start=2):  # start=2 porque linha 1 é header
            try:
                ...
                failed += 1
                error_msg = {"row": idx, "data": row, "error": str(e)}
    ...
    def _create_or_update_contact(
        self, row: Dict, group: ContactGroup = None
    ) -> Contact:
        """
        ...
        if not name or not phone:
            raise ValueError(
                f"Campos obrigatórios faltando: name={name}, phone={phone}"
            )
...
def sync_contacts_from_users():
    ...
    users_with_phone = (
        User.objects.filter(is_active=True, profile__phone_number__isnull=False)
